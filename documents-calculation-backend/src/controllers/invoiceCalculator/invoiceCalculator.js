...
exports.parseData = (fileToRead) => {
    ...
    const validated = invoices.every(invoice => {
        ...
        if (invoice['Parent document'] === '' ||
        // Or the parent document is included in the invoices' document number
        invoices.map(i => i['Document number']).includes(invoice['Parent document']))
            return true
        else {
    ...
    else throw `Invoice number ${faultyInvoice['Document number']}'s Parent number is not available in the data you have provided.`
...
exports.parseCurrencies = (currencies) => {
    ...
    if (Object.keys(currencies).some(currency => !allowedCurrencies.includes(currency))) {
...
exports.getTotal = (data, currencies, outputCurrency, vat) => {
    ...
    if (vat)
        data = data.filter(invoice => invoice['Vat number'] === vat)
    ...
    data.forEach(invoice => {
        ...
        if (Object.keys(output).includes(vatNum)) {
            ...
            } else throw 'A customer can only have one VAT number - please make sure that is the case in your data'
        }
        else {
            output[vatNum] = {
    ...
    for (const invoice in output) {
        listOutput.push(`Customer ${ output[invoice]['Customer']} - ${output[invoice]['Total']} ${outputCurrency}`)
    }
    return listOutput
}
